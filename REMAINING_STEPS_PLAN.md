...
- [x] **Decision locked (2026-09-14):** this is exactly the scenario `Overall-plan.md` §10's fairness rule anticipated, and the 9B model failed it outright — a strawman, not a fair cheap-model baseline. Per the rule, the model steps up. Confirmed via a separate planning session (see `nemotron_ultra_findings.md` for the discussion; the decision itself is now recorded in `Overall-plan.md` §10/§19/§20 as canonical, not just in that transcript): **step up to a 35B-class model** — candidates `Holo-3.5-35B`, `Qwen-3.8-27B`, `Gemma-4-26B`, `Ornith-1.5-35B`. Also confirmed: the project is on **PRISM's Free tier**.
- [ ] **Ali, next action:** bring up one of the 35B candidates, validate it clears Category A cleanly (100% correct tool calls, zero held/frozen state triggered on any clean-control call) before reporting any number — if a candidate fails, try the next one, don't silently downgrade the bar. Say the model-size jump plainly on stage per §10's honesty note (it's bigger than "one size," and it softens the small-model-cost framing — that trade is stated, not hidden).
- [ ] Once Category A passes reliably: run `use_mock=False` end-to-end for Categories B/E/F (the ones that actually differentiate v0/v1/v2) so v1's prompt fix gets a genuine chance to show partial improvement, instead of being architecturally identical to v0 (Phase 0 finding).
