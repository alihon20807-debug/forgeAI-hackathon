...
`Slides-Plan.md` is the original build spec (historical — written before the deck existed). `DECK-CONTENT.md` is the current, accurate record of what's actually in the built deck right now. All prior known issues (a stale "~15B" model reference, count mismatches, unmeasured "5-second" timing claims, an invented "₹250 CR" figure, and present-tense future sectors) have been fully audited and resolved across both `claimguard-pitch.html` and `claimguard-pitch-offline.html`.
