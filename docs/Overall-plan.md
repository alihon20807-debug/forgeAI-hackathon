...
- **Inference backend options (Local vs. Google AI Studio):**
  - *Local llama-server*: Fully offline, zero API quota constraints on Ali's GPU machine.
  - *Google AI Studio (Gemma ~31B / `gemma-2-27b-it` via OpenAI proxy route)*: Recommended for faster, more reliable testing under tight deadlines without local GPU thermal/compilation friction. *Caution:* Free-tier quotas apply (e.g. 15 RPM / daily limits); bounded smoke runs (`--limit 3`) or mock fallback must be used for rapid iteration to avoid 429 quota exhaustion.
...
- **P3 — Evidence & Pitch:** the Replay Set and its labels; the local checker; the PRISM project (tracing, tagging, the credit-managed run plan); screenshots, charts, and the pitch materials themselves (the 6-slide deck built per `docs/presentation/Slides-Plan.md`).
