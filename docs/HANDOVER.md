...
> - **Safety Fallback & Fast-Testing Alternative:** If local hardware encounters venue or thermal issues, or to accelerate testing under tight timelines, use **Google AI Studio with Gemma ~31B** (e.g. `gemma-2-27b-it`) via its OpenAI-compatible endpoint (`https://generativelanguage.googleapis.com/v1beta/openai/`). It eliminates local model setup overhead while providing reliable tool-calling. *Caution:* Free-tier quotas apply (e.g. 15 RPM / daily limits); use bounded runs (`--limit 3`) or mock fallback during rapid testing to avoid 429 quota exhaustion. Hosted fallback via PRISM's proxy or LiteLLM is also supported.
>
...
```json
{
  ...
  "redacted_pii": [
    { "type": "CARD_NUMBER", "matched": "4111 1111 1111 1111", "valid_luhn": true }
  ]
}
```
